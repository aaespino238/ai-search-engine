...
import { ResponseBlock, Source } from "./utils/constants";
...
export default function Home() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const responseBlock = await response.json() as ResponseBlock
      console.log("RESPONSE BLOCK: ", responseBlock)
      setMessages(prev => [...prev, { role: "ai" as const, content: responseBlock.response }])
      
      // TODO: Add sources to the UI
      const sourceBlock = responseBlock.sources.map((source: Source) => {
        return `**${source.page_title}** \n ${source.url} \n ${source.relevant_information}`
      }).join("\n\n")
      setMessages(prev => [...prev, { role: "ai" as const, content: sourceBlock }])
